...
struct ruta {
    ...
    ruta() {
        memset(ip, 0, 4); // Inicializar la IP a ceros
        memset(puerto, 0, 10); // Inicializar el puerto a ceros
        TTL = 255; // Valor máximo del TTL para indicar que no ha sido actualizado
    }
};
#endif